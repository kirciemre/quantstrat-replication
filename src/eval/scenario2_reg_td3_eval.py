"""
Evaluation for the scenario2 reg-TD3 experiment (see scenario2_reg_td3_train.py).

At test time we only need the actor (deterministic policy) -- the critics
were only needed during training to shape the actor's gradients. Structure
mirrors scenario2_reg_eval.py exactly so the two are directly comparable.
"""
...
# --- scatter: chosen inventory vs signal ---
...
plt.title(f"Policy: inventory vs signal (reg-TD3, scenario 2, run {args.run_id})")
plt.tight_layout()
plt.savefig(f"policy_scatter_scenario2_td3_run{args.run_id}.png", dpi=130)
plt.close()

import os
os.makedirs("artifacts", exist_ok=True)
np.save(f"artifacts/rewards_scenario2_reg_td3_run{args.run_id}.npy", episode_rewards)
